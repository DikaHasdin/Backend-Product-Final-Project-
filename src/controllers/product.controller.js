...
const readDaftarSizeProduct = async (req, res) => {
    const idProduct = req.params.id
    const allProduct = await knex.table('size_product').innerJoin('size', 'size.id', '=', 'size_product.id_size').innerJoin('warna_product', 'warna_product.id', '=', 'size_product.id_warna_product').where('warna_product.id_product',idProduct)
...
const createAddToBag = async (req, res) => {
    const {id_warna_product, id_size} = req.body
    ...
    return res.status(201).send({
        message: "Create Add to Bag succes"
    })
}
...
const createFavourite = async (req, res) => {
    ...
    return res.status(201).send({
        message: "Create Favourite succes"
    })
}
